...
fun NoiseRouter.barrier(barrier: Argument.DensityFunction) = apply { this.barrier = densityFunctionOrDouble(barrier) }
fun NoiseRouter.barrier(barrier: Double) = apply { this.barrier = densityFunctionOrDouble(barrier) }

fun NoiseRouter.fluidLevelFloodedness(fluidLevelFloodedness: Argument.DensityFunction) =
...
fun NoiseRouter.fluidLevelSpread(fluidLevelSpread: Argument.DensityFunction) =
...
fun NoiseRouter.lava(lava: Argument.DensityFunction) = apply { this.lava = densityFunctionOrDouble(lava) }
fun NoiseRouter.lava(lava: Double) = apply { this.lava = densityFunctionOrDouble(lava) }

fun NoiseRouter.temperature(temperature: Argument.DensityFunction) = apply { this.temperature = densityFunctionOrDouble(temperature) }
fun NoiseRouter.temperature(temperature: Double) = apply { this.temperature = densityFunctionOrDouble(temperature) }

fun NoiseRouter.vegetation(vegetation: Argument.DensityFunction) = apply { this.vegetation = densityFunctionOrDouble(vegetation) }
fun NoiseRouter.vegetation(vegetation: Double) = apply { this.vegetation = densityFunctionOrDouble(vegetation) }

fun NoiseRouter.continents(continents: Argument.DensityFunction) = apply { this.continents = densityFunctionOrDouble(continents) }
fun NoiseRouter.continents(continents: Double) = apply { this.continents = densityFunctionOrDouble(continents) }

fun NoiseRouter.erosion(erosion: Argument.DensityFunction) = apply { this.erosion = densityFunctionOrDouble(erosion) }
fun NoiseRouter.erosion(erosion: Double) = apply { this.erosion = densityFunctionOrDouble(erosion) }

fun NoiseRouter.depth(depth: Argument.DensityFunction) = apply { this.depth = densityFunctionOrDouble(depth) }
fun NoiseRouter.depth(depth: Double) = apply { this.depth = densityFunctionOrDouble(depth) }

fun NoiseRouter.ridges(ridges: Argument.DensityFunction) = apply { this.ridges = densityFunctionOrDouble(ridges) }
fun NoiseRouter.ridges(ridges: Double) = apply { this.ridges = densityFunctionOrDouble(ridges) }

fun NoiseRouter.initialDensityWithoutJaggedness(initialDensityWithoutJaggedness: Argument.DensityFunction) =
...
fun NoiseRouter.finalDensity(finalDensity: Argument.DensityFunction) = apply { this.finalDensity = densityFunctionOrDouble(finalDensity) }
fun NoiseRouter.finalDensity(finalDensity: Double) = apply { this.finalDensity = densityFunctionOrDouble(finalDensity) }

fun NoiseRouter.veinToggle(veinToggle: Argument.DensityFunction) = apply { this.veinToggle = densityFunctionOrDouble(veinToggle) }
fun NoiseRouter.veinToggle(veinToggle: Double) = apply { this.veinToggle = densityFunctionOrDouble(veinToggle) }

fun NoiseRouter.veinRidged(veinRidged: Argument.DensityFunction) = apply { this.veinRidged = densityFunctionOrDouble(veinRidged) }
fun NoiseRouter.veinRidged(veinRidged: Double) = apply { this.veinRidged = densityFunctionOrDouble(veinRidged) }

fun NoiseRouter.veinGap(veinGap: Argument.DensityFunction) = apply { this.veinGap = densityFunctionOrDouble(veinGap) }
