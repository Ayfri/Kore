...
data class InventoryManager<T : ContainerArgument>(val container: T) {
	...
	fun clearAll(item: ItemArgument) = when (container) {
		...
		is Vec3 -> fn.data(container)
			.remove("Items[{id:\"${item.asId()}\",${item.components?.let { "components:${it.asJson()}" } ?: ""}}]")
				...
				execute {
					...
					run {
						...
						if (type == SlotEventType.DURING_TAKEN) {
							slotListener.events.filter { it.type == SlotEventType.WHEN_TAKEN }
								.forEachIndexed { index, whenTakenEvent ->
