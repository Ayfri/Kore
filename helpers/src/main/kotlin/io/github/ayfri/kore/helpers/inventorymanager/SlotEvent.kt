...
context(fn: Function)
fun SlotEventListener.duringTake(block: Function.() -> Unit) =
	with(fn.datapack) { event(SlotEventType.DURING_TAKEN, block) }
...
context(fn: Function)
fun SlotEventListener.onceTaken(block: Function.() -> Unit) =
	with(fn.datapack) { event(SlotEventType.ONCE_TAKEN, block) }
