...
private fun approximateGivensQuaternion(
	diagonalElement1: Float,
	offDiagonalElement: Float,
	diagonalElement2: Float
): GivensPair {
	val f = 2.0f * (diagonalElement1 - diagonalElement2)
	return when {
		COTANGENT_PI_OVER_8 * offDiagonalElement * offDiagonalElement < f * f -> GivensPair.normalize(
			offDiagonalElement,
			f
		)
...
private fun applyJacobiIteration(
	...
	if (currentMatrix.m01 * currentMatrix.m01 + currentMatrix.m10 * currentMatrix.m10 > 1.0E-6f) {
		givensPair = approximateGivensQuaternion(
			currentMatrix.m00,
			0.5f * (currentMatrix.m01 + currentMatrix.m10),
			currentMatrix.m11
		)
	...
	if (currentMatrix.m02 * currentMatrix.m02 + currentMatrix.m20 * currentMatrix.m20 > 1.0E-6f) {
		givensPair =
			approximateGivensQuaternion(
				currentMatrix.m00,
				0.5f * (currentMatrix.m02 + currentMatrix.m20),
				currentMatrix.m22
			).negateSin()
	...
	if (currentMatrix.m12 * currentMatrix.m12 + currentMatrix.m21 * currentMatrix.m21 > 1.0E-6f) {
		givensPair = approximateGivensQuaternion(
			currentMatrix.m11,
			0.5f * (currentMatrix.m12 + currentMatrix.m21),
			currentMatrix.m22
		)
